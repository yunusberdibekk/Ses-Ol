...
final class AuthService {
    ...
    func citizienSignup(method: String, citizienName: String, citizienSurname: String, citizienPassword: String, citizienPhone: String, citizienCountry: String, citizienCity: String, citizienDistrict: String, citizienFullAddress: String) async -> Result<CitizienSignupResponse, AuthError> {
        let validateResult = AuthValidator.shared.citizienSignupValidator(
            citizienName: citizienName,
            citizienSurname: citizienSurname,
            citizienPassword: citizienPassword,
            citizienPhone: citizienPhone,
            citizienCountry: citizienCountry,
            citizienCity: citizienCity,
            citizienDistrict: citizienDistrict,
            citizienFullAddress: citizienFullAddress)
        if let validateResult = validateResult {
            .failure(validateResult)
            return
        }
        let result = await NetworkManager.shared.auth(url: .usersCrud, method: .post, model: CitizienSignupRequest(
            method: method,
            user_tel: citizienPhone,
            user_password: citizienPassword,
            user_name: citizienName,
            user_surname: citizienSurname,
            address_district: citizienDistrict,
            address_city: citizienCity,
            address_country: citizienCountry,
